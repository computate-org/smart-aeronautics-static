...
function getMapBox(container, token, zoom, center, pitch, antialias=true, style='mapbox://styles/mapbox/standard') {
...
function updateMapBoxModel(result, index, solrIdVar, locationVar, altitudeMetersVar, pitchVar, yawVar, rollVar, rotationVar, gltfPathVar) {
  // parameters to ensure the model is georeferenced correctly on the map
  const modelOrigin = [result[locationVar].coordinates[0], result[locationVar].coordinates[1]];
  const modelAltitude = result[altitudeMetersVar];
  const modelRotate = result[rotationVar];
  ...
  const layerId = result[solrIdVar];
  var customLayer = map.getLayer(layerId);
  if(!customLayer) {
    customLayer = {
      id: layerId,
      type: 'custom',
      ...
      onAdd: function (map, gl) {
        ...
        loader.load(
          window.STATIC_BASE_URL + result[gltfPathVar],
          (gltf) => {
